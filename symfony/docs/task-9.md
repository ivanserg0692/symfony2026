...
В рамках задачи выполнены multi-stage реорганизация PHP image, production-настройка OPcache, разделение dev/prod/load-test targets, выделение отдельных Redis-инстансов прикладного кеша, подготовка production-like load-test окружения и инструментов корреляции Symfony profiler с SQL. В итоге проект имеет три стандартных runtime-профиля: production по умолчанию для боевой работы, изолированный `load_test` для тестов производительности без development-диагностики и development с подробными логами приложения, Symfony profiler и Xdebug. Для их автоматической активации предусмотрены npm-команды `set:prod`, `set:dev` и `set:load-test`, которые загружают нужные environment-файлы в отдельном интерактивном shell. Для каталога проверен режим look-ahead пагинации: запрос загружает `limit + 1` строку и возвращает `hasNextPage`, что позволяет не выполнять точный `COUNT` на основном пути списка. Режим управляется feature flag и сохраняет прежний вариант ответа при отключении.
...
The task delivered a multi-stage PHP image reorganization, production OPcache configuration, separate dev/prod/load-test targets, dedicated application Redis instances, a production-like load-test environment, and tooling that correlates Symfony profiler records with SQL. The project now has three standard runtime profiles: production by default for regular traffic, isolated `load_test` for performance measurements without development diagnostics, and development with detailed application logs, the Symfony profiler, and Xdebug. The `set:prod`, `set:dev`, and `set:load-test` npm commands automate activation by loading the required environment files in a separate interactive shell. A catalog look-ahead pagination mode was tested: it fetches `limit + 1` rows and returns `hasNextPage`, avoiding an exact `COUNT` on the common listing path. A feature flag controls the mode and preserves the previous response variant when disabled.
